...
export function getCategories(): Category[] {
  ...
  // Only include Array, String, and Linked List categories
  const allowedCategories = [
    { name: "Array", data: arrayProblemsData },
    { name: "String", data: stringProblemsData },
    { name: "Linked List", data: linkedlistProblemsData }
  ]
  
  allowedCategories.forEach(({ name, data }) => {
    const problems = (data as any).problems.map((problem: any) => castProblem({
      ...problem,
      category: name,
      status: "unsolved"
    }))
...
export function getProblemsByCategory(categoryId: string): Problem[] {
  ...
  if (categoryName.toLowerCase() === "array") {
    return (arrayProblemsData as any).problems.map((problem: any) => castProblem({
      ...problem,
      category: "Array",
      status: "unsolved"
    }))
...
export function getProblemById(id: number): Problem | undefined {
  ...
  const arrayProblem = (arrayProblemsData as any).problems.find((p: any) => p.id === id)
  if (arrayProblem) {
    return castProblem({
      ...arrayProblem,
      category: "Array",
...
export function getUserProgress(): Record<number, "Unsolved" | "In-progress" | "Solved"> {
  if (typeof window === 'undefined') return {}
  const stored = localStorage.getItem('userProgress')
  const rawProgress = stored ? JSON.parse(stored) : {}
  
  // Convert boolean values to status strings for backward compatibility
  const convertedProgress: Record<number, "Unsolved" | "In-progress" | "Solved"> = {}
  for (const [key, value] of Object.entries(rawProgress)) {
    const problemId = parseInt(key)
    if (typeof value === 'boolean') {
      convertedProgress[problemId] = value ? "Solved" : "In-progress"
    } else {
      convertedProgress[problemId] = value as "Unsolved" | "In-progress" | "Solved"
    }
  }
...
export function updateUserProgress(problemId: number, status: "Unsolved" | "In-progress" | "Solved") {
  if (typeof window === 'undefined') return
...
// Backward compatibility function for boolean updates
export function updateUserProgressBoolean(problemId: number, completed: boolean) {
  updateUserProgress(problemId, completed ? "Solved" : "In-progress")
}
...
export function getCategoryStats() {
  ...
  return categories.map(category => {
    ...
    const inProgress = problems.filter(p => progress[p.id] === "In-progress").length
...
export function getOverallStats() {
  ...
  const solved = allProblems.filter(p => progress[p.id] === "Solved").length
  const inProgress = allProblems.filter(p => progress[p.id] === "In-progress").length
